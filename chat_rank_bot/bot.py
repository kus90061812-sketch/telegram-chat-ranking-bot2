...
class RankingBot:
    ...
    async def handle_message(
        ...
        if command in HELP_COMMANDS:
            ...
            min_text_length, ranking_limit = await asyncio.gather(
                asyncio.to_thread(get_min_text_length, self.storage, chat.id),
                asyncio.to_thread(get_ranking_limit, self.storage, chat.id),
            )
            await message.reply_text(
                help_message(
    ...
    async def count_message(self, update: Update) -> None:
        ...
        min_text_length = await asyncio.to_thread(
            get_min_text_length, self.storage, chat.id
        )
        if not is_countable_text(text, min_text_length):
            return
    ...
    async def show_daily(self, update: Update) -> None:
        ...
        entries, ranking_limit = await asyncio.gather(
            asyncio.to_thread(self.storage.rankings, chat.id, "day", keys.day_key),
            asyncio.to_thread(get_ranking_limit, self.storage, chat.id),
        )
        await message.reply_text(
            daily_ranking_message(entries, keys.day_key, ranking_limit),
            parse_mode=ParseMode.HTML,
            link_preview_options=NO_LINK_PREVIEW,
        )
    ...
    async def show_weekly(self, update: Update) -> None:
        ...
        if not message or not chat:
            return
        keys = period_keys(datetime.now(timezone.utc), self.settings.timezone)
        entries, chat_cfg = await asyncio.gather(
            asyncio.to_thread(self.storage.rankings, chat.id, "week", keys.week_key),
            asyncio.to_thread(get_event_settings, self.storage, chat.id),
        )
        await message.reply_text(
            weekly_ranking_message(
                entries, keys.week_key, chat_cfg.ranking_limit, chat_cfg.ranking_footer
            ),
            parse_mode=ParseMode.HTML,
            link_preview_options=NO_LINK_PREVIEW,
        )
    ...
    async def show_event(self, update: Update) -> None:
        message = update.effective_message
        chat = update.effective_chat
        if not message or not chat:
            return
        event = await asyncio.to_thread(get_event_settings, self.storage, chat.id)
        await message.reply_text(
            event_message(event.event_emoji, event.event_title, event.event_body),
            link_preview_options=NO_LINK_PREVIEW,
        )
    ...
    async def show_me(self, update: Update) -> None:
        ...
        if not message or not chat or not user:
            return
        keys = period_keys(datetime.now(timezone.utc), self.settings.timezone)
        daily, weekly, min_text_length = await asyncio.gather(
            asyncio.to_thread(
        ...
        await message.reply_text(
            personal_message(daily, weekly, min_text_length),
            parse_mode=ParseMode.HTML,
        )
        ...
        for chat_id in chat_ids:
            try:
                entries, chat_cfg = await asyncio.gather(
                    ...
                    asyncio.to_thread(get_event_settings, self.storage, chat_id),
                )
                await application.bot.send_message(
                    chat_id=chat_id,
                    text=finalized_weekly_ranking_message(
                        entries, week_key, chat_cfg.ranking_limit, chat_cfg.ranking_footer
                    ),
                    parse_mode=ParseMode.HTML,
                    link_preview_options=NO_LINK_PREVIEW,
                )
