...
class FormattingTests(unittest.TestCase):
    ...
    def test_event_message_preserves_newlines(self) -> None:
    ...
    def test_ranking_limit_can_show_one_to_ten(self) -> None:
        result = daily_ranking_message([], "2026-09-07", ranking_limit=10)
        self.assertIn("1위 - 집계 없음 [ 0회 ]", result)
        self.assertIn("10위 - 집계 없음 [ 0회 ]", result)
    ...
    def test_weekly_footer_preserves_newlines_and_links_usernames(self) -> None:
        footer = "다음 주 월요일 오후 6시 마감\n문의 : @new_manager\n추가 안내"
        result = weekly_ranking_message([], "2026-08-17", ranking_footer=footer)
        self.assertIn("<b>다음 주 월요일 오후 6시 마감</b>", result)
        self.assertIn(
            '문의 : <a href="https://t.me/new_manager">@new_manager</a>',
            result,
        )
        self.assertIn("\n추가 안내", result)
